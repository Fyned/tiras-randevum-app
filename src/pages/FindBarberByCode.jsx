...
const FindBarberByCode = () => {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            ...
            if (queryError || !data) {
                 if (queryError && queryError.code === 'PGRST116') { // single() returns no rows
                    setNotFound(true);
                } else {
                    throw queryError || new Error("Berber verisi alınamadı.");
                }
            } else if (!data.is_public || !data.is_active) {
                setNotFound(true);
            } else {
    ...
    if (profile.role !== 'customer') {
        return (
            <div className="min-h-[calc(100vh-4rem)] bg-[color:var(--tr-bg)] flex items-center justify-center">
                <p className="text-sm text-[color:var(--tr-text-muted)]">Bu sayfaya sadece müşteri hesapları erişebilir.</p>
            </div>
        );
    }
                ...
                >
                    <div className="p-8 rounded-2xl bg-[color:var(--tr-bg-soft)] border border-[color:var(--tr-border-strong)] space-y-6">
                        <div className="text-center">
                            <h1 className="text-2xl font-bold text-[color:var(--tr-text)]">Berberini Bul</h1>
                            <p className="text-[color:var(--tr-text-muted)] mt-2 text-sm">Berberinizin size verdiği kodu girerek profiline ulaşın.</p>
                        </div>
                        <form onSubmit={handleSearch} className="space-y-4">
                            <div>
                                <Label htmlFor="barberCode" className="sr-only">Berber Kodu</Label>
                                <Input
                                    id="barberCode"
                                    type="text"
                            ...
                            <Button type="submit" disabled={loading} className="w-full h-12 text-base bg-[color:var(--tr-accent)] text-white hover:bg-[color:var(--tr-accent)]/90 disabled:opacity-50">
                                {loading ? (
                                    <>
